...
function selectPreferredTranscript(
  transcripts?: Transcript[] | null,
  fallback?: Transcript | null
): Transcript | null {
  if (transcripts && transcripts.length > 0) {
    return transcripts.find((t) => !t.isAutoGenerated) ?? transcripts[0];
  }
...
export default function PlayerScreen() {
  ...
  const dbVideo = useMemo<Video | undefined>(() => {
    if (!id) return undefined;
    const storedVideo = videoRepo.getVideoWithTranscripts(id);
    if (!storedVideo) return undefined;

    const preferredTranscript = selectPreferredTranscript(storedVideo.transcripts);
  ...
  const localTranscript = useMemo(
    () =>
      selectPreferredTranscript(
        libraryVideo?.transcripts ?? dbVideo?.transcripts ?? null,
        libraryVideo?.transcript ?? dbVideo?.transcript ?? null
      ),
    [
      libraryVideo?.transcript,
      libraryVideo?.transcripts,
      dbVideo?.transcript,
      dbVideo?.transcripts,
    ]
  );
  const localDescription = useMemo(
    () => normalizeDescription(libraryVideo?.description ?? dbVideo?.description),
  ...
  useEffect(() => {
    setStreamingTranscript(localTranscript);
  }, [id, localTranscript]);

  useEffect(() => {
    setVideoDescription(localDescription);
  }, [id, localDescription]);
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((meta) => {
        ...
        setVideoDescription(normalizedDescription);
  ...
  // Load transcript from desktop only when local cache is missing.
  useEffect(() => {
    if (!id) return;
    if (localTranscript?.segments?.length) {
    ...
    api
      ...
      .then((transcripts) => {
        ...
        if (transcripts.length > 0) {
          ...
          const preferred = selectPreferredTranscript(transcripts);
  ...
  const handleWordPress = useCallback(
    async (word: string, segmentTimestamp: number) => {
      ...
      const cachedTranslation = wordsRepo.getCachedTranslationLocal({
        sourceText: cleanWord,
        targetLang,
      });

      setSelectedWord(cleanWord);
      setWordTranslation(cachedTranslation ?? null);
      setIsTranslating(!cachedTranslation);
      setWordSaved(false);
      ...
      try {
        ...
        wordsRepo.cacheTranslationLocal({
